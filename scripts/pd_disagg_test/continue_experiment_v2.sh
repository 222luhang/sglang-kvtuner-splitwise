...
# 继续两种模型对比实验 (跳过已完成的配置)
#
# 用法: bash continue_experiment.sh
# ============================================================================
...
run_experiment() {
    ...
    if [ -f "$output_csv" ]; then
        local count=$(tail -n +2 "$output_csv" | wc -l | tr -d ' ')
        if [ "$count" -ge 30 ]; then
            log_info "Skipping ${model_name}_${exp_name} (already completed: ${count} rows)"
            return 0
        fi
    fi
...
# 主流程
log_step "Continuing two-model comparison experiment"
