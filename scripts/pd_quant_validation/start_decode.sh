#!/bin/bash
# start_decode.sh - 启动 Decode 服务（带 KVTuner 量化）
...
KV_TUNER_NBITS_SCALE="${KV_TUNER_NBITS_SCALE:-1.2}"  # Decode 使用更高精度

# KVTuner 量化配置
KV_CACHE_DTYPE="${KV_CACHE_DTYPE:-fp8_e5m2}"
ENABLE_KVTUNER="${ENABLE_KVTUNER:-true}"
# 如果使用层级量化，这些参数会被配置文件覆盖
KV_TUNER_NBITS_KEY="${KV_TUNER_NBITS_KEY:-8}"
KV_TUNER_NBITS_VALUE="${KV_TUNER_NBITS_VALUE:-8}"
KV_TUNER_RESIDUAL_LENGTH="${KV_TUNER_RESIDUAL_LENGTH:-32}"
...
if [ "$ENABLE_KVTUNER" = "true" ]; then
    ...
    # Decode 模式特定配置
    export SGLANG_KVTUNER_DECODE_NBITS_SCALE=${KV_TUNER_NBITS_SCALE}
fi

# 其他优化选项
CMD="$CMD --mem-fraction-static 0.8"
CMD="$CMD --max-running-requests 128"  # Decode 通常运行更多并发请求
...
echo "$CMD"
echo ""
echo "日志文件：$LOG_DIR/decode_$(date +%Y%m%d_%H%M%S).log"
echo ""

# 启动服务
nohup bash -c "$CMD" > "$LOG_DIR/decode_$(date +%Y%m%d_%H%M%S).log" 2>&1 &
PID=$!

echo "Decode 服务已启动 (PID: $PID)"
echo ""
