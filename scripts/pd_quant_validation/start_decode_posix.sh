#!/bin/bash
# start_decode_posix.sh - 启动 Decode 服务（POSIX 后端 + KVTuner 层级量化）
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/decode_posix_$(date +%Y%m%d_%H%M%S).log"

# ==================== 打印配置 ====================

echo "=========================================="
echo "  Decode 服务启动 (POSIX 后端)"
echo "=========================================="
...
export KVTUNER_DECODE_NBITS_SCALE="${KV_TUNER_NBITS_SCALE}"
...
CMD="$CMD --max-running-requests 128"
